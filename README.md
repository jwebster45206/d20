...
- **Fluent rolls**: named modifiers and advantage/disadvantage
...
Map keys are lowercase by convention. `SkillCheck`, `StrikeRoll`, and `D100SkillCheck` lowercase the *query*; they do not rewrite stored keys.
...
wizard.Attributes = map[string]int{
    "intelligence": 18, // ability score; SkillCheck would add +18 if used as a skill
    "athletics":    5,  // skill bonus used by SkillCheck
}
...
`SkillCheck` adds the stored attribute value to 1d20. The library does not derive 5e ability modifiers; store the bonus you want (`athletics: 5`, not `strength: 16`) unless you intend the raw score.

### Modifiers and rolls

`Modifiers` are not derived from `Attributes`. `StrikeRoll` applies only the keys you pass; missing keys are skipped. Situational extras go on the `DiceManager`. `RollPercentile` ignores advantage/disadvantage.
...
`Dice(2, 10).RollPercentile()` and `D100SkillCheck` implement compatible d100 roll-under mechanics (tens + ones, `00` = 100). They are not a Call of Cthulhu rules implementation. Call of Cthulhu® is a registered trademark of Chaosium Inc.
